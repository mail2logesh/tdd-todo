...
do
  UNMERGED_COMMIT_MESSAGE=$(echo "$UNMERGED_COMMIT" | sed -e 's/^ *//g' -e 's/ *$//g')
  MATCHED=false;
  for IGNORED_COMMIT in $IGNORED_COMMITS
  do
        case "$UNMERGED_COMMIT_MESSAGE" in
  ...
  then
    ...
    COMMIT_FOUND_IN_TO_BRANCH=$(git log $TO_BRANCH --oneline | grep $ESCAPED_COMMIT_MESSAGE)
    if test "$COMMIT_FOUND_IN_TO_BRANCH" = ""
    then
      UNMERGED_COMMIT_COUNT=$((UNMERGED_COMMIT_COUNT+1));
      echo "$UNMERGED_COMMIT_MESSAGE"
    fi
  fi
done
echo "########################################################"
